...
import Name
import GHC.Unicode
import Development.IDE.Spans.Common (emptySpanDoc, spanDocToMarkdown)
...
getCompletions
    ...
    -> WithSnippets
    -> IO [CompletionItem]
getCompletions ideOpts ideState CC{..} _maybe_parsed (localBindings, bmapping) prefixInfo caps withSnippets = do
...
toggleSnippets ClientCapabilities {_textDocument} (WithSnippets with) x
  ...
  where
    supported = Just True == (_textDocument >>= _completion >>= _completionItem >>= _snippetSupport)
